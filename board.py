    ...
    [
        ...
        dbc.Row(
            dbc.Col(
                [
                    dcc.Upload(
                        [html.Div([html.H5("Arraste os arquivos aqui, ou clique")])],
                        id="upload",
        ...
        dbc.Row(
            [
                dbc.Col(dbc.Jumbotron(id="tab")),  # Final da coluna 1
                dbc.Col(dbc.Jumbotron(dcc.Graph(id="grafico"))),  # Final da coluna 2
            ]
        ),  # Final da linha 2
    ],
...
@app.callback(
    Output("store", "data"), Input("upload", "contents"), State("store", "data"))
...
@app.callback(Output("grafico", "figure"), Input("store", "data"))
def upload(dat):
    if dat is not None:
